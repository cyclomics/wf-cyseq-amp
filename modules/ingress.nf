...
workflow ingress {
    ...
    main:
        ...
        def barcodePattern = asPattern(getMinKnowBarcodeFolderPattern(params.barcodes))
        def runFolderPattern = asPattern(getMinKnowAutoRunFolderPattern())

        log.info "Looking for FASTQ files with pattern: ${read_pattern}"
        log.info "Only selecting barcodes in pattern: ${barcodePattern}"
...
def getMinKnowBarcodeFolderPattern(barcodes) {
    if (!barcodes?.trim()) {
        return '^barcode\\d{2,3}$|^unclassified$'
    }

    def barcode_list = barcodes
        .split(',', -1)
        .collect { it -> it.trim() }

    def invalid_barcodes = barcode_list.findAll {
        it -> !(it ==~ /^barcode\d{2,3}$/)
    }

    if (invalid_barcodes) {
        error(
            "Invalid barcode(s): ${invalid_barcodes.join(', ')}. " +
            "Expected barcode names matching 'barcodeNN' or 'barcodeNNN', " +
            "for example: 'barcode01,barcode02'."
        )
    }

    def barcode_pattern = barcode_list
        .collect { it -> java.util.regex.Pattern.quote(it) }
        .join('|')
