...
workflow {
    ...
    // 2. Consensus
    generate_cyseq_consensus(FilterShortReads.out, ch_reference)
    consensus_bam = generate_cyseq_consensus.out.bam
    consensus_folder = generate_cyseq_consensus.out.consensus_folder
    // TODO: Use plots generated by Cycas

    // 3. Alignment
    // align_consensus_reads(consensus_bam, consensus_folder, ch_reference, minimap2_mode, ch_regions)
    ...
    if (params.regions != 'auto') {
        ...
        depth_table = GetAmpliconDepth(consensus_bam, ch_regions.collect())
    } else {
    ...
    merge_consensus_alignments(grouped_consensus_bam, ch_regions)
